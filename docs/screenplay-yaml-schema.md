...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | 稳定人物标识，供场景、对白和辅助检查引用。 |
| `name` | string | 人物姓名。 |
...
设计原因：剧本创作需要持续追踪人物动机和功能。单独人物表可以避免每个场景重复描述角色，也方便人物共现线索作为辅助检查使用，但 Schema 主体仍服务于剧本 YAML 初稿。
...
2. 保留 `id` 字段：即使作者修改标题或姓名，后续场景引用、辅助人物检查和版本追踪也更稳定。
